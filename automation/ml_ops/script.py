...
interactions_df.sort_values(
    "timestamp", axis=0, ascending=True, inplace=True, na_position="last"
)
...
interactions_df.to_csv(
    ("./domain/Media/data/Interactions/" + interactions_filename),
    index=False,
    float_format="%.0f",
)
...
itemmetadata_df.rename(
    columns={"genres": "GENRE", "movieId": "ITEM_ID", "year": "YEAR"}, inplace=True
)
itemmetadata_filename = "item-meta.csv"
itemmetadata_df.to_csv(
    ("./domain/Media/data/Items/" + itemmetadata_filename),
    index=False,
    float_format="%.0f",
)
